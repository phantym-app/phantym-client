...
const Video = ({ video }: Props) => {
    ...
    return (
      <div class={styles.progressbar}>
        <input
          ...
          onMouseUp={() => wasPlaying && setVideoDetails({ ...videoDetails, isPaused: false })}
          ref={videoHandle}
          class={styles.handle}
          type={'range'}
  ...
  const PlayPause = () => {
    return (
      <button
        onMouseDown={() =>
          videoDetails.isPaused || videoDetails.currentTime === videoDetails.duration
            ? setVideoDetails({
                ...videoDetails,
                isPaused: false,
              })
        ...
        <img
          src={videoDetails.currentTime === videoDetails.duration ? repeat : videoDetails.isPaused ? play : pause}
          alt={videoDetails.currentTime === videoDetails.duration ? 'repeat' : videoDetails.isPaused ? 'play' : 'pause'}
        />
      </button>
    );
  };
    ...
    return (
      <div class={styles.volumeButton}>
        <button
          ...
          <img
            src={isMuted ? muted : noVolume ? muted : lowVolume ? volumeLow : mediumVolume ? volumeMedium : volumeHigh}
            alt={'volume'}
          />
        </button>
  ...
  const ResizeScreen = () => {
    ...
    return (
      <button
        onMouseDown={() =>
          isFullscreen
            ? (document.exitFullscreen(), setVideoDetails({ ...videoDetails, isFullscreen: false }))
            : (openFullscreen(), setVideoDetails({ ...videoDetails, isFullscreen: true }))
        }>
